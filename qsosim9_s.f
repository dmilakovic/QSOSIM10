...
c ----------------------------------------------------------------------------
	SUBROUTINE QSOSIM9(zqso,alpha,vmag,wstart,wend,dw,nc,nuplim,
     ...
     :          flerr,nnflux,npoints,numlin,ni,nhi4,z4)
	  ...
	  real*8, dimension(numlin) :: dummy, dummyy
...
c         write(*,*)wda(i),flux(i),noise(i),nnflux(i)
	...
	call PGENV(11.9,22.1,1.9,1.02*real(zqso),0,1)
	call pgpt(numlin,real(nhi4),real(z4),2)
	call PGENV(11.9,22.1,0.0,180.0,0,1)
	call pghist(numlin,real(nhi4),12.0,22.0,200,1)
c	call PGENV(1.0,real(numlin),11.5,22.5,0,1)
c	call PGLABEL('Line number','log NHI','Random choice of NHI')
c	call PGLINE(numlin,real(dummy),real(nhi4))
      call pgend
