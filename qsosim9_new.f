...
c ----------------------------------------------------------------------------
      SUBROUTINE QSOSIM9_new(home,zqso,alpha,r_mag,wstart,wend,dw,nuplim,
     :          sigblur,inoise,npts,loglam,flux,flerr,nnflux,flux_nc,
     :          npoints,numlin,ni,nhi4,z4)
	real*4 wda(262144),danoabs(262144)
	...
	character*20 cMed
	CHARACTER :: home*120
	real*8 lognhi,nhi,b,z,select,median
c       real*4 w,ff,sum,zstart,zend,z1p1,z2p1,zleft,zright
c       real*4 a12p5,rn,a13p75,a13p1,mbp1,zqsop1,p,q,r,s,
	real*4 c,d,vlight,pi
c       real*8  delta,mdelta
c       real*4 nhills(20),blls(20),zlls(20)
	real*8, intent(in)::zqso,alpha,r_mag,wstart,wend,dw,nuplim
	real*8, intent(in)::sigblur
	integer, intent(in) :: inoise,npts,numlin
	real*8, dimension(numlin), intent(in)::nhi4,z4
	...
	real*8 ran3, dark,gain,ron2,sig,t,pixsize,area
	real*4 n1,n2,n3
...
c nc is the column density cut-off (REAL, NOT log10)
	zqsop1=zqso+1
	c = 3631*1e-23*((vlight*1e3)/(0.6182e-6)**2)*1e-10 ! conversion 10e-23 erg s-1 cm-2 Hz-1 --> erg s-1 cm-2 A-1
	c = alog10(c)/0.4
c	write (6,*) c
