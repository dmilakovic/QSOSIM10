...
c            write(*,*) "Line in channel no.",chand
	    ...
	    j=jmid
*     
            do while (tauchan.gt.1.0e-6.and.j.ge.1.and.j.le.npts)
c               
               if(jstep.eq.-1) then
                  if(j.gt.2) then
                     j=j-1
                     ...
                     tauchan=1.0e-7
                     if(jmid.le.1) jmid=2!1
c                     write (6,*) 'jstep,j,jmid',jstep,j,jmid,' goto 901'
                     goto 901
                  end if
	       else
                  if(j.lt.npts-1) then
...
c               write (6,*) 'ww, wv, bl, a',ww,wv,bl
c               write (6,*) 'v,a',wv*wv*(1.0d0/ww-1.0d0/wv)/bl,a
               do klv=1,nexbin
                  fluxcur=flx(j)
...
*                   this seems to be voigt profile...
                  subtau=real(cne*voigt( v, a ))
                    ...
                    write (6,*) 'v is NaN!, jstep,nexbin =',jstep,nexbin
        write (6,*)'j,jmid,wlod,klv,dxd',j,jmid,wlod,dble(klv),0.5d0*dxd
        write (6,*) 'ww, wav(j+1),(j),(j-1)',ww,wav(j+1),wav(j),wav(j-1)
                     stop
                     subtau=0.0
                  end if
                  e = expf(-subtau)
...
*	          add as simple sub-chunks
               ...
               tauchan=stau
 901           continue
               if(jstep.lt.0.and.(tauchan.lt.1.0e-6.or.j.le.2)) then
                  jstep=1
                  j=jmid-1
                  tauchan=1.0e25
               end if
